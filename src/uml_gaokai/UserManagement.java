...
public class UserManagement extends JDialog {
  ...
  private void jbInit() throws Exception {
    ...
    com.borland.dx.sql.dataset.QueryDescriptor q=new com.borland.dx.sql.dataset.QueryDescriptor(database1, "SELECT user.userID,user.userName,user.userPassword,user.userAddress,user.userAuthority,use" +
          "r.userGender,user.userAge FROM user", null, true,Load.ALL);
    queryDataSet1.setQuery(q);
//    queryDataSet1.setQuery(new QueryDescriptor(database1, "SELECT [user].userID,[user].userName,[user].userPassword,[user].userAddress,[user].userAuthority,[use" +
